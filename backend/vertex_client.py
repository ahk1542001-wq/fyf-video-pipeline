...
def vertex_client_kwargs(
    *,
    ...
    """Prefer an explicit Vertex Express key; otherwise use standard ADC."""
    load_dotenv(
        dotenv_path=env_file or Path(__file__).resolve().parents[1] / ".env",
        override=False,
    )
    api_key = os.getenv("FYF_VERTEX_API_KEY") or os.getenv("GOOGLE_API_KEY")
    if api_key:
        return {"vertexai": True, "api_key": api_key}
    local_credentials = credential_file or Path(__file__).resolve().parents[1] / "gcp-key.json"
    if not os.getenv("GOOGLE_APPLICATION_CREDENTIALS") and local_credentials.is_file():
